  ...
  return (
    <header className="nav">
      <div className="maxw nav-in">
        <a className="nav-logo" href="#" onClick={(e) => { e.preventDefault(); setOpen(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>WBI</a>
        <nav className="nav-links">
          {links.map(([l, id]) => (
            <a key={l} className="nav-link" href={'#' + id} onClick={(e) => { e.preventDefault(); go(id); }}>{l}</a>
          ))}
        </nav>
      ...
      {open && (
        <div className="nav-drawer">
          {links.map(([l, id]) => (
            <a key={l} className="nav-drawer-link" href={'#' + id} onClick={(e) => { e.preventDefault(); go(id); }}>{l}</a>
          ))}
